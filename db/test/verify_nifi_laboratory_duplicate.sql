...
BEGIN
    ...
    IF NOT FOUND
       ...
       OR v_run.duplicate_count <> 1 THEN
        RAISE EXCEPTION 'NiFi valid-file replay was not a duplicate';
    END IF;
    ...
    IF NOT FOUND
       ...
       OR v_run.rejected_count <> 0
       OR v_run.duplicate_count <> 1 THEN
        RAISE EXCEPTION 'NiFi initial-file replay was not a duplicate';
    END IF;

    SELECT count(*) INTO v_count FROM ods.laboratory_result;
    IF v_count <> 2 THEN
        RAISE EXCEPTION 'NiFi duplicate replay changed ODS count to %', v_count;
    END IF;
    ...
    IF v_count <> 1 THEN
        RAISE EXCEPTION 'NiFi duplicate replay changed replay history count to %', v_count;
    END IF;
END;
$$;
